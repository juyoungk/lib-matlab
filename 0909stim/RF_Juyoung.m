...
function p =  ParseInput(varargin)
    ...
    addParamValue(p,'DisplayRes', [800,600], @(x) length(x)==2);
    addParamValue(p,'DisplayRate', 60, @(x) x>=30);
    addParamValue(p,'objContrast', 1, @(x) x>=0 && x<=1);
    addParamValue(p,'seed', 1, @(x) isnumeric(x));
    %
    addParamValue(p,'movieDurationSecs', 300, @(x)x>0);
    ...
    addParamValue(p,'stimSizeXYum', 1600*[1 1], @(x) all(size(x)==[1 2]) && all(x>0));
    %
