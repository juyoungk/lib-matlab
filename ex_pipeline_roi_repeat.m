...
%% ROI selection
    ...
    img_for_ROI = g.(ex_str)(i_file_for_ROI).AI_mean{1};
...
%% Given the ROIs, intensity over time for all sessions
    ch_save = 1; % CH selection for ROI trace plot
    smoothing_size = 12;
    ...
    for i_ex = 1:n_experiments
        % Experiment (or recording) parameters
            ...
            % string for name (ex or recording file)
            s_loc = strfind(g.(ex_str)(i_ex).tif_filename, ex_str);
            s_loc_end = strfind(g.(ex_str)(i_ex).tif_filename, '.tif');
            s_exp_name = g.(ex_str)(i_ex).tif_filename(s_loc:s_loc_end-1);
            s_exp_name = [ex_str,' ',g.(ex_str)(i_ex).tif_filename(s_loc+length(ex_str):s_loc_end-1)];
        ...
        for k=1:numStimulus
            ...
            hold on;
                    ...
                    ax.YAxis.FontSize = Fontsize;
                    ax.XLim = [0 interval];
                    title(['Mean responses of ROIs: ', s_exp_name],'FontSize',18);   
                    % legend
